...
from models.ikun import MeMOTR_IKUN
...
def train_one_epoch(model: MeMOTR_IKUN, train_states: dict, max_norm: float,
    ...
    model.train()
    optimizer.zero_grad()
    device = next(get_model(model).memotr_model.parameters()).device
    ...
    for i, batch in enumerate(dataloader):
        ...
        tracks = TrackInstances.init_tracks(batch=batch,
                                            hidden_dim=get_model(model).memotr_model.hidden_dim,
                                            num_classes=get_model(model).memotr_model.num_classes,
