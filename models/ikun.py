        ...
        if self.config["KUM_MODE"] and (epoch >= self.config["TG_EPOCH"]):
            if self.config["KUM_MODE"] == 'cascade attention':
                visual_feat = self.visual_local_global(
                    x['local_img'], x['global_img'], textual_hidden, self.config["KUM_MODE"]
                )
            elif self.config["KUM_MODE"] in ['cross correlation', 'text-first modulation']:
                visual_feat = self.visual_local_global(
                    x['local_img'], x['global_img'], textual_feat, self.config["KUM_MODE"]
                )
        else:
...
def build(memotr_model:MeMOTR,config: dict):
    return MeMOTR_IKUN(memotr_model=memotr_model,config=config)